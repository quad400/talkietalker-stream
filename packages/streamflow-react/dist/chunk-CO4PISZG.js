...
function brandingToTheme(branding) {
  ...
  return {
    ...
    showBranding: branding.show_streamflow_badge ?? true,
...
// src/client/streamflow-client.ts
...
var StreamFlowClient = class _StreamFlowClient {
  publishKey;
  tokenPath;
  fetchImpl;
  ...
  static create(options) {
    if (!options || Object.keys(options).length === 0) {
      if (!_StreamFlowClient.instance) {
        _StreamFlowClient.instance = new _StreamFlowClient();
      }
      return _StreamFlowClient.instance;
    }
    return new _StreamFlowClient(options);
  }
  getPublishKey() {
...
var defaultLabelsEn = {
  ...
  poweredBy: "Powered by StreamFlow",
...
// src/components/streamflow.tsx
import * as React from "react";
import { jsx } from "react/jsx-runtime";
var StreamFlowClientContext = React.createContext(null);
function StreamFlow({
  children,
  client,
  clientOptions,
  ...
  return /* @__PURE__ */ jsx(StreamFlowClientContext.Provider, { value: resolvedClient, children: /* @__PURE__ */ jsx(
    "div",
    {
...
var StreamFlowProvider = StreamFlow;
...
function isEmbedBridgeEvent(data) {
  ...
  return type === "streamflow:joined" || type === "streamflow:left" || type === "streamflow:error";
}

// src/room/streamflow-room.tsx
...
function StreamFlowRoom({
  ...
  React3.useEffect(() => {
    ...
    void client.getConnection(roomId, {
      ...
      if (!cancelled) {
        setConnection(info);
        setStreamFlowConfig({ wsUrl: info.wsUrl });
      }
    }).catch((err) => {
      if (!cancelled) {
  ...
  const handleLeave = React3.useCallback(() => {
    session.leave();
    postEmbedEvent({ type: "streamflow:left", payload: { reason: "user" } });
    onLeave?.();
  }, [session, onLeave]);
...
//# sourceMappingURL=chunk-CO4PISZG.js.map