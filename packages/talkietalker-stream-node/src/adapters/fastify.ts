...
import { createTalkieTalkerStreamHandlers, type HandlerOptions } from '../handlers/index.js'

export type FastifyPluginConfig = HandlerOptions & {
  prefix?: string
}

export async function talkieTalkerStreamPlugin(
  app: FastifyInstance,
  ...
  app.post(`${prefix}/token`, async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const result = await handlers.handleToken(
        ...
        { publishKey: req.headers['x-talkietalker-stream-publish-key'] as string | undefined },
      )
  ...
  app.post(`${prefix}/webhooks`, {
    ...
    try {
      ...
      const signature = (req.headers['x-talkietalker-stream-signature'] as string | undefined) ?? ''
