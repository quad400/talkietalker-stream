...
export function talkieTalkerStreamHandlers(options: HandlerOptions = {}) {
    ...
    try {
      if (route === TOKEN_SEGMENT) {
        ...
        const result = await handlers.handleToken(
          ...
          { publishKey: req.headers.get('X-TalkieTalker-Stream-Publish-Key') },
        )
      ...
      if (route === WEBHOOK_SEGMENT) {
        ...
        const signature = req.headers.get('X-TalkieTalker-Stream-Signature') ?? ''
