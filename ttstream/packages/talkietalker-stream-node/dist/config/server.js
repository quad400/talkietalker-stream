import { ENV_NAMES, isPublishableKey, isSecretKey, isWebhookSecret, resolveBaseUrl, } from './endpoints.js';
...
export function resolveServerConfig(overrides) {
    ...
    const publishKey = overrides?.publishKey ??
        readEnv(ENV_NAMES.publishKey) ??
        readEnv(ENV_NAMES.publishKeyPublic);
    if (publishKey && !isPublishableKey(publishKey)) {
        throw new ConfigurationError('publishKey must start with pk_test_ or pk_live_.');
    }
...
export function defineTalkieTalkerStreamConfig(config) {
    return config;
}
